...
int DVBS2_DECODE::decode_ts_frame( scmplx* pl )
{
	...
	else
	{
		...
		m_ldpc_gpu.decode_soft( &pl[90], N0, m_payload_symbols, nSymbolSize, m_format[0].constellation + 2,
			...
			m_soft_bits, m_soft_bits_cache, m_bitLDPC );// 0.7 ms
#endif
		sdkStopTimer( &timerStep );
		timerStepValue[nTimeStep++] = sdkGetTimerValue( &timerStep );
#endif
	...
	sdkStopTimer( &timerStep );
	timerStepValue[nTimeStep++] = sdkGetTimerValue( &timerStep );// 0.7 ms 
...
void DVBS2_DECODE::set_configure()
{
	...
	memcpy_s( m_Symbols[M_QPSK], 60*sizeof(scmplx), m_qpsk, 4*sizeof(scmplx) );
...
#ifdef USE_GPU
	m_ldpc_gpu.updateSymbolsTemplate( m_Symbols[0] );
#endif	
